...
- `src/components/player/` — Custom HLS player (ported from upstream KVideo). `VideoPlayer.tsx` is the shell (proxy/retry state machine + progress persistence) → `CustomVideoPlayer` (device dispatch) → `DesktopVideoPlayer` (orchestrator) with a full custom control layer in `desktop/`. State is a `refs`/`data`/`actions` container (`hooks/useDesktopPlayerState`) wired by `hooks/useDesktopPlayerLogic` over domain hooks in `hooks/desktop/` (playback, volume, progress, skip, fullscreen, controls-visibility, shortcuts, cast). `hooks/useHlsPlayer.ts` owns hls.js (HEVC H.264 level-locking, iOS blob ad-filter with fallback, unsupported-env direct→proxy fallback); `hooks/useAutoSkip.ts` (intro/outro/auto-next, decoupled), `hooks/useStallDetection.ts`. Ad filtering via `lib/utils/m3u8-utils.ts` (heuristic/keyword/aggressive). `src/components/ui/Icon.tsx` shims the player's `Icons.*` to `lucide-react`. Danmaku, watch-together, and the Google Cast SDK are intentionally not wired.
