...
export function useAutoSkip({
  ...
  const [isTransitioningToNextEpisode, setIsTransitioningToNextEpisode] =
    useState(false)
  ...
  const triggerNextEpisode = useCallback(
    (reason: string) => {
      ...
      if (lastHandledSrcRef.current === src) {
        console.log(
          `[AutoSkip] Ignoring ${reason} trigger: already handled for this source`,
        )
        return
      }
      ...
      if (isTransitioningToNextEpisode) {
        console.log(
          `[AutoSkip] Ignoring ${reason} trigger: already transitioning`,
        )
        return
      }
  ...
  const attemptIntroSkip = useCallback(() => {
    ...
    if (
      ...
      if (video.readyState >= 2) {
        ...
        console.log(
          `[AutoSkip] Jumping from ${currentTime}s to intro skip point ${skipIntroSeconds}s`,
        )
  ...
  useEffect(() => {
    ...
    const inOutroZone =
      remainingTime > 0 && remainingTime <= skipOutroSeconds && currentTime > 0
    ...
    if (inOutroZone) {
      ...
      if (isPlaying) {
        console.log(
          `[AutoSkip] Outro detected: ${remainingTime.toFixed(1)}s remaining`,
        )
