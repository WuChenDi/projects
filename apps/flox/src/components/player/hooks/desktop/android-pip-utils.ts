...
export function createAndroidPiPTransitionPlan(
  fullscreenMode: FullscreenMode,
): AndroidPiPTransitionPlan {
...
export function shouldRollbackTemporaryWindowFullscreen(
  session: AndroidPiPSessionState | null,
): boolean {
  return Boolean(session?.enteredTemporaryWindowFullscreen)
}

export function getAndroidPiPSourceRect(
  container: HTMLElement | null,
): AndroidPiPSourceRect | null {
  if (!container) {
    return null
  }
