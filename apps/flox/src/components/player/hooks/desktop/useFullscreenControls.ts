...
import type { AndroidPiPSessionState } from './android-pip-utils'
import {
...
export function useFullscreenControls({
  ...
  const getAndroidPiPBridge = useCallback((): AndroidPiPBridge | null => {
    ...
    const bridge = (window as Window & { FloxAndroid?: AndroidPiPBridge })
      .FloxAndroid
  ...
  useEffect(() => {
    if (typeof document !== 'undefined') {
      ...
      const hasAndroidPiPBridge = Boolean(
        androidBridge?.isPictureInPictureSupported?.(),
      )
      setIsPiPSupported(hasNativePiP || !!hasWebkitPiP || hasAndroidPiPBridge)
    }
  }, [getAndroidPiPBridge, setIsPiPSupported, videoRef])
  ...
  const requestAndroidPictureInPicture = useCallback(async () => {
    ...
    if (
      !bridge ||
      !video ||
      typeof bridge.enterPictureInPicture !== 'function'
    ) {
      return false
    }
    ...
    try {
      ...
      const height =
        video.videoHeight || containerRef.current?.clientHeight || 9
  ...
  useEffect(() => {
    ...
    window.addEventListener('flox-android-pip-change', handleAndroidPiPChange)
    return () =>
      window.removeEventListener(
        'flox-android-pip-change',
        handleAndroidPiPChange,
      )
  }, [exitWindowFullscreen])
  ...
  const togglePictureInPicture = useCallback(async () => {
    ...
    try {
      if (
        fullscreenDocument.pictureInPictureElement &&
        fullscreenDocument.exitPictureInPicture
      ) {
        ...
        fullscreenDocument.pictureInPictureEnabled
      ) {
