...
interface UseSkipControlsProps {
  ...
  skipForwardTimeoutRef: React.RefObject<NodeJS.Timeout | null>
  skipBackwardTimeoutRef: React.RefObject<NodeJS.Timeout | null>
}
...
export function useSkipControls({
  ...
  const skipForward = useCallback(() => {
    ...
    const newSkipAmount = showSkipForwardIndicator
      ? skipForwardAmount + seekStepSeconds
      : seekStepSeconds
    ...
    const targetTime = Math.min(
      videoRef.current.currentTime + seekStepSeconds,
      duration,
    )
  ...
  const skipBackward = useCallback(() => {
    ...
    const newSkipAmount = showSkipBackwardIndicator
      ? skipBackwardAmount + seekStepSeconds
      : seekStepSeconds
