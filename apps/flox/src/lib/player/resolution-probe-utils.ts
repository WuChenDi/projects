...
const TEXT_QUALITY_PATTERNS: Array<{
  ...
  {
    pattern: /(?:^|[^\d])(1080p?|1080i|fhd|fullhd|full-hd)(?:[^\d]|$)/i,
    width: 1920,
    height: 1080,
    label: '1080P',
    color: 'bg-green-500',
...
export function getResolutionLabel(
  width: number,
  height: number,
): ResolutionProbeLabel {
  const w = Math.max(width, height)
  const h = Math.min(width, height)
  if (h >= 4320)
    return { width: w, height: h, label: '8K', color: 'bg-rose-500' }
  if (h >= 2160)
    return { width: w, height: h, label: '4K', color: 'bg-amber-500' }
  if (h >= 1440)
    return { width: w, height: h, label: '2K', color: 'bg-emerald-500' }
  if (h >= 1080)
    return { width: w, height: h, label: '1080P', color: 'bg-green-500' }
  if (h >= 720)
    return { width: w, height: h, label: '720P', color: 'bg-teal-500' }
  if (h >= 540)
    return { width: w, height: h, label: '540P', color: 'bg-cyan-500' }
  if (h >= 480)
    return { width: w, height: h, label: '480P', color: 'bg-sky-500' }
  if (h >= 360)
    return { width: w, height: h, label: '360P', color: 'bg-gray-500' }
  if (h >= 240)
    return { width: w, height: h, label: '240P', color: 'bg-gray-500' }
  if (h >= 144)
    return { width: w, height: h, label: '144P', color: 'bg-gray-500' }
...
export function chooseHigherQuality(
  ...
  return getCandidateRank(candidate) > getCandidateRank(current)
    ? candidate
    : current
}

export function extractResolutionHint(
...
export function extractVariantPlaylistUrls(
  content: string,
  baseUrl: string,
): string[] {
