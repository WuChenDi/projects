 ...
 */
export function useSourceBadges<
  T extends { source?: string; sourceName?: string },
>(videos: T[], availableSources: SourceBadge[]) {
  ...
  const filteredVideos = useMemo(() => {
    ...
    return videos.filter(
      (video) => video.source && selectedSources.has(video.source),
    )
