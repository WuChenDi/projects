...
export function usePremiumHomePage() {
  ...
  const [enabledPremiumSources, setEnabledPremiumSources] = useState<
    VideoSource[]
  >([])
  ...
  const executeSearch = useCallback(
    (searchQuery: string, sources: VideoSource[]) => {
      ...
      void performSearch(searchQuery, sources, currentSortBy as any)
  ...
  useEffect(() => {
    ...
    if (urlQuery) {
      ...
      const currentSources = currentSettings.premiumSources.filter(
        (s) => s.enabled,
      )
