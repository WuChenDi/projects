        ...
        results.forEach((result, index) => {
          ...
          if (result.status === 'fulfilled') {
            ...
            if (fetchResult.normalSources.length > 0) {
              currentSources = mergeSources(
                currentSources,
                fetchResult.normalSources,
              )
              anyChanged = true
            }
            ...
            if (fetchResult.premiumSources.length > 0) {
              currentPremiumSources = mergeSources(
                ...
                fetchResult.premiumSources,
              )
              anyChanged = true
            }

            // Update timestamp for successful sync
            const subIdx = updatedSubscriptions.findIndex(
              (s) => s.id === sub.id,
            )
            if (subIdx !== -1) {
            ...
            console.error(
              `Failed to sync subscription: ${sub.name}`,
              result.reason,
            )
          }
        })
