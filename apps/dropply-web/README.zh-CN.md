...
- **客户端加密**（`src/lib/crypto.ts`）
  - 文件和文本在离开浏览器前即完成 AES-GCM 加密，密钥经 Argon2id 派生（`@cdlab/cipher`）
...
- **加密** — `@cdlab/cipher`（AES-GCM + Argon2id，ECIES）、`@cdlab/uncrypto`（跨运行时加密垫片）
...
| 路径 | 职责 |
|---|---|
| `src/lib/crypto.ts` | 密钥生成、URL fragment 编解码、文件与文本的加解密（封装 `@cdlab/cipher`） |
