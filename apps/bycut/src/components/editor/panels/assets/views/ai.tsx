...
import type { AIGenerationHistoryEntry } from '@/stores/ai-generation-history-store'
import {
  getHistoryImageBlob,
...
import { useAIImageGenerationStore } from '@/stores/ai-image-generation-store'
import { useAISettingsStore } from '@/stores/ai-settings-store'
import type { GeneratedVideo } from '@/stores/ai-video-generation-store'
import { useAIVideoGenerationStore } from '@/stores/ai-video-generation-store'
    ...
    return (
      <div className={cn('group/ref relative', className)}>
        ...
        <button
          ...
          title={t('ai.removeReferenceImage')}
          onClick={(event) => {
      ...
      <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
        ...
        <div className="flex flex-col gap-1">
          <p className="text-foreground text-sm font-medium">
            {t('ai.noImageProvider')}
          </p>
          <p className="text-muted-foreground text-xs">{t('ai.getStarted')}</p>
        </div>
        <Button
          type="button"
          variant="outline"
...
function AssetStatusBadge({
  ...
  if (status === 'pending' || status === 'adding') {
    return (
      <div className="absolute top-1 right-1" title={t('misc.addingToAssets')}>
  ...
  if (status === 'failed') {
    return (
      <button
        ...
        title={t('misc.addToAssetsFailedRetry')}
        onClick={(event) => {
          event.stopPropagation()
          onRetry()
        }}
...
function VideoStatusBadge({
  ...
  if (video.assetStatus === 'added') {
    return (
      <span className="text-xs text-green-600">{t('misc.addedToAssets')}</span>
    )
  }
...
function HistoryEntryCard({ entry }: { entry: AIGenerationHistoryEntry }) {
  ...
  const hasExternalUrl = entry.url.length > 0 && !entry.url.startsWith('data:')
  ...
  return (
    <div className="bg-muted/50 group relative overflow-hidden rounded-md border">
      <div className="relative aspect-square w-full overflow-hidden">
        ...
        >
          <Trash2 className="size-4 drop-shadow" />
        </button>
        {isLoaded && (
          <button
            type="button"
...
function AIHistoryView() {
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
        <p className="text-muted-foreground text-sm">{t('ai.noHistory')}</p>
        <p className="text-muted-foreground text-xs">
          {t('ai.resultsPlaceholder')}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <span className="text-muted-foreground text-xs font-medium">
          {t('common.numItems', { num: entries.length })}
        </span>
        <Button
          type="button"
          variant="ghost"
      ...
      {imageEntries.length > 0 && (
        <div className="flex flex-col gap-2">
          <span className="text-muted-foreground text-xs font-medium">
            {t('ai.imagesCount', { num: imageEntries.length })}
          </span>
          <div className="grid grid-cols-2 gap-2">
      ...
      {videoEntries.length > 0 && (
        <div className="flex flex-col gap-2">
          <span className="text-muted-foreground text-xs font-medium">
            {t('ai.videosCount', { num: videoEntries.length })}
          </span>
          <div className="flex flex-col gap-2">
  ...
  return (
    <>
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <span className="text-muted-foreground text-xs font-medium">
            {t('characters.numCharacters', { num: characters.length })}
          </span>
          ...
          <div className="flex flex-col gap-2">
            {characters.map((character) => (
              <button
                ...
                onKeyDown={(event) => {
                  if (event.key === 'Enter') setViewingCharacter(character)
                }}
              >
                ...
                <div className="flex min-w-0 flex-col gap-0.5">
                  ...
                  <span className="text-muted-foreground text-[10px]">
                    {t('ai.numImages', { num: character.images.length })}
                  </span>
                </div>
              </button>
            ))}
  ...
  return (
    <Tabs defaultValue="image" className="-mt-2 flex h-full flex-col">
      <TabsList className="mb-3 justify-start">
        ...
        <TabsTrigger value="video">{t('common.video')}</TabsTrigger>
      </TabsList>
      <TabsContent value="image" className="mt-0 flex min-h-0 flex-1 flex-col">
        <AIImageView />
      </TabsContent>
      <TabsContent value="video" className="mt-0 flex min-h-0 flex-1 flex-col">
        <AIVideoView />
      </TabsContent>
    </Tabs>
  )
