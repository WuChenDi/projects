...
import type { TransitionPreset } from '@/constants/transition-constants'
import {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b px-4 pt-3 pb-2">
        <h3 className="mb-2 text-sm font-medium">{t('assets.transitions')}</h3>
        <p className="text-muted-foreground mb-2 text-xs">
          {t('transitions.instructions')}
        </p>
...
function applyTransitionToAdjacentPairs({
  ...
  if (applied === 0) {
    toast.info(t('transitions.noAdjacentClips'))
  } else {
    toast.success(
      t('transitions.applied', {
        type: transitionType,
        num: applied,
      }),
    )
