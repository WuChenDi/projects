...
export const useSoundsStore = create<SoundsStore>((set, get) => ({
  ...
  saveSoundEffect: async ({ soundEffect }) => {
    try {
      ...
      toast.error(i18next.t('sounds.saveFailed'))
  ...
  removeSavedSound: async ({ soundId }) => {
    try {
      ...
      set({ savedSoundsError: errorMessage })
      toast.error(i18next.t('sounds.removeFailed'))
  ...
  clearSavedSounds: async () => {
    try {
      ...
      set({ savedSoundsError: errorMessage })
      toast.error(i18next.t('sounds.clearFailed'))
  ...
  addSoundToTimeline: async ({ sound }) => {
    ...
    if (!audioUrl) {
      toast.error(i18next.t('sounds.fileNotAvailable'))
      return false
    }
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : i18next.t('sounds.addToTimelineFailed'),
        { id: `sound-${sound.id}` },
      )
      return false
    }
