...
async function removeCharacterImageBlob({ id }: { id: string }): Promise<void> {
  try {
    const db = await openCharacterImageDB()
    await new Promise<void>((resolve, reject) => {
      const tx = db.transaction([CHARACTER_IMAGES_STORE], 'readwrite')
...
interface CharacterStoreState {
  ...
  addCharacter: (params: { name: string; description: string }) => string
  ...
  addImage: (params: { characterId: string; image: CharacterImage }) => void

  removeImage: (params: { characterId: string; imageId: string }) => void
  ...
  getCharacterByName: (params: { name: string }) => AICharacter | undefined
}
  ...
  persist(
    (set, get) => ({
      ...
      removeImage: ({ characterId, imageId }) => {
        ...
        set((state) => ({
          characters: state.characters.map((c) => {
            ...
            const updatedImages = c.images.filter((i) => i.id !== imageId)
            return {
              ...c,
        ...
        set((state) => ({
          characters: state.characters.map((character) =>
            character.id === characterId
              ? {
                  ...character,
                  generations: [generation, ...character.generations],
                  updatedAt: new Date().toISOString(),
                }
              : character,
          ),
      ...
      getCharacterByName: ({ name }) => {
        ...
        return get().characters.find((c) => c.name.toLowerCase() === lowerName)
      },
    }),
    {
      name: 'ai-characters',
    ...
    characters: state.characters.map((c) => {
      ...
      return {
        ...
        images: c.images.map((img) =>
          img.id === firstImage.id ? { ...img, referenceUrl: url } : img,
        ),
      }
    }),
  }))
