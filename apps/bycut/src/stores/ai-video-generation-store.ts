...
export const useAIVideoGenerationStore = create<AIVideoGenerationState>()(
  (set, get) => ({
    ...
    generate: async () => {
      ...
      if (!trimmedPrompt) {
        toast.error(i18next.t('ai.enterPrompt'))
        return
      }
      ...
      if (isDevPlaceholderActive()) {
        ...
        try {
          ...
          const project = editor.project.getActiveOrNull()
          if (!project) {
            toast.error(i18next.t('editor.noActiveProject'))
          ...
          const message =
            error instanceof Error ? error.message : i18next.t('ai.videoFailed')
          toast.error(message)
          set({ isGenerating: false })
        }
        return
      ...
      if (!videoProviderId) {
        toast.error(i18next.t('ai.configureVideoProvider'))
        return
      }
      ...
      try {
        ...
        toast.success(i18next.t('ai.videoSubmitted'))
  ...
  try {
    ...
    if (finalResult.status === 'succeeded' && finalResult.videoUrl) {
      ...
      updateVideo({
        ...
        updates: {
          taskStatus: 'failed',
          error: finalResult.error ?? i18next.t('ai.videoFailed'),
        },
      })
      toast.error(finalResult.error ?? i18next.t('ai.videoFailed'))
    }
  } catch (error) {
    const message =
      error instanceof Error ? error.message : i18next.t('ai.videoCheckFailed')
    updateVideo({
      videoId,
