...
export function useEditorActions() {
  ...
  useActionHandler(
    ...
    () => {
      ...
      toast.loading(
        i18next.t('tts.converting', {
          count: textElements.length,
        }),
        { id: toastId },
      )

      // biome-ignore lint/nursery/noFloatingPromises: We want this to run without awaiting it
      ;(async () => {
        let successCount = 0
        let failCount = 0
